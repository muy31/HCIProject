...
function draw() {
    ...
    drawSun(sunCanvas.width / 2, 3 * sunCanvas.height / 7, sunCanvas.width * 0.25, 8, sunCanvas.width * 0.37, ctx, dotangle);
    ctx.fillStyle = "black";
    ctx.font = "bold 14px Arial";
    ctx.fillText("Hover over me", sunCanvas.width/9, 6.5 * sunCanvas.height / 9, 7*sunCanvas.width / 9);
    ctx.fillText("to return to the", sunCanvas.width / 9, 7.5 * sunCanvas.height / 9, 7 * sunCanvas.width / 9);
    ctx.fillText("previous page!", sunCanvas.width / 9, 8.5 * sunCanvas.height / 9, 7 * sunCanvas.width / 9);
...
const sunTime = { timeLeft: 3, on: false };

//Button callbacks
sunCanvas.addEventListener('mouseover', () => {
    // Handle exit action
    sunCanvas.style.backgroundColor = "darkblue";
    sunTime.on = true;
    continueTimer(sunTime, sunCanvas, sessionStorage.getItem("previousPage"));
});

sunCanvas.addEventListener('mouseleave', () => {
    sunCanvas.style.backgroundColor = "lightskyblue";
    sunCanvas.innerHTML = "";
    sunTime.timeLeft = 3;
    sunTime.on = false;
});